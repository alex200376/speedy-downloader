import i18n from "i18next";
import { initReactI18next } from "react-i18next";

const resources = {
  en: {
    translation: {
      appName: "SpeedDownloader",
      nav: {
        all: "All",
        downloading: "Active",
        completed: "Completed",
        paused: "Paused",
        error: "Failed",
      },
      header: {
        newDownload: "New Download",
        search: "Search downloads…",
        playlist: "Playlist Download",
      },
      update: {
        title: "Check for updates",
        checking: "Checking for updates…",
        upToDate: "You're up to date",
        downloadNow: "A new version is available",
        currentVersion: "Current version",
        latestVersion: "Latest version",
        releaseNotes: "Release notes",
        downloadUpdate: "Download update",
        downloadStarted: "Update package download started",
        openPage: "Open release page",
        retry: "Retry",
        failed: "Couldn't check for updates",
      },
      action: {
        pause: "Pause",
        resume: "Resume",
        cancel: "Cancel",
        remove: "Remove",
        openFolder: "Open folder",
        retry: "Retry",
        start: "Start Download",
        download: "Download",
        browse: "Browse",
        copy: "Copy",
        close: "Close",
        save: "Save",
        verify: "Verify checksum",
      },
      status: {
        Pending: "Confirming",
        Queued: "Queued",
        Downloading: "Downloading",
        Paused: "Paused",
        Completed: "Completed",
        Error: "Error",
        Canceled: "Canceled",
      },
      task: {
        segments: "{{n}} connections",
        unknownSize: "Unknown size",
        error: "Error",
        hash: "Hash",
        hashMatched: "Checksum matches",
        hashMismatch: "Checksum mismatch",
      },
      error: {
        forbidden: "The server returned 403 (likely blocked by Cloudflare / anti-bot)",
        notFound: "File not found (404)",
        invalidUrl: "Please enter a valid http(s) URL",
        stalled: "Connection stalled with no data (timeout)",
        cancelled: "Download was cancelled",
        taskMissing: "Task not found",
      },
      verify: {
        algorithm: "Algorithm",
        expectedHash: "Expected hash",
        optional: "leave empty to auto-detect from sidecar file",
        computing: "Computing hash…",
      },
      dialog: {
        title: "New Download",
        grabTitle: "Confirm Download",
        url: "Download URL",
        urlPlaceholder: "https://example.com/file.zip",
        filename: "File name",
        filenameHint: "Auto-detect from server",
        saveTo: "Save to",
        segments: "Connections",
        referer: "Referer (optional)",
        customHeaders: "Custom headers (optional)",
        videoQuality: "Video quality",
        writeSubs: "Subtitles",
        enableSubs: "Enable subtitle download",
        detectSubs: "Auto-detect",
        invalidUrl: "Please enter a valid http(s) URL",
        added: "Download task added",
      },

      playlist: {
        title: "Playlist Download",
        fetch: "Fetch",
        found: "{{n}} videos found",
        noVideos: "No videos found in playlist",
        selectAll: "Select all",
        deselectAll: "Deselect all",
        selected: "selected",
        downloadSelected: "Download selected",
        added: "{{n}} videos added to download queue",
        hint: "Paste a YouTube playlist or channel URL above",
      },
      settings: {
        title: "Settings",
        saveDir: "Download folder",
        maxConcurrent: "Max concurrent downloads",
        defaultSegments: "Default connections per file",
        speedLimit: "Speed limit",
        speedLimitKbps: "KB/s (0 = unlimited)",
        proxy: "Proxy",
        proxySystem: "System",
        proxyNone: "Direct",
        proxyCustom: "Custom",
        proxyHint: "Supports http(s):// and socks5:// (e.g. http://127.0.0.1:7890)",
        language: "Language",
        theme: "Theme",
        accent: "Accent color",
        appearance: "Appearance",
        downloads: "Downloads",
        behavior: "Behavior",
        duplicatePolicy: "When file already exists",
        dupRename: "Rename (add suffix)",
        dupOverwrite: "Overwrite",
        dupSkip: "Skip download",
        sortByType: "Sort downloads into type subfolders (Videos/Audio/Documents/…)",
        notifyComplete: "Show a notification when a download finishes",
        openFolderOnComplete: "Auto-open the folder after download finishes",
        themeDark: "Dark",
        themeLight: "Light",
        extension: "Chrome Extension",
        extensionDesc:
          "Install the extension to grab downloads directly from your browser.",
        extensionSteps:
          "1. Download the extension zip · 2. Extract it · 3. Open chrome://extensions · 4. Enable Developer mode · 5. Load unpacked → select the extracted folder",
        extensionInstall: "Install extension",
        extensionDownload: "Download from GitHub",
        extensionInstalled:
          "Extension folder is ready and copied to clipboard. Click “Load unpacked” in the browser page and paste.",
        extensionFail: "Failed to prepare extension",
        extensionOpenFail: "Failed to open the extensions page (Chrome/Edge not found)",
        extensionNoBrowser: "not detected",
        extensionBrowsers: "Detected browsers:",
        openExtensionsPage: "Open extensions page",
        extensionPath: "Extension folder",
        apiToken: "API token",
        apiTokenHint: "(paste into the browser extension)",
        copyToken: "Copy",
        apiTokenCopied: "API token copied",
        saved: "Settings saved",
      },
      theme: {
        dark: "Dark",
        light: "Light",
        system: "System",
      },
      videoQuality: {
        best: "Best (video+audio)",
        "2160p": "4K (2160p)",
        "1080p": "1080p",
        "720p": "720p",
        "480p": "480p",
        "360p": "360p",
        video: "Video only",
        audio: "Audio only",
      },
      empty: {
        title: "No downloads yet",
        hint: "Click “New Download” or use the Chrome extension to grab files from the web.",
      },
      drop: {
        hint: "Drop link to add a download",
      },
      toast: {
        paused: "Download paused",
        resumed: "Download resumed",
        canceled: "Download canceled",
        removed: "Task removed",
        error: "Operation failed",
        saved: "Settings saved",
      },
      title: {
        speed: "Speed",
        downloaded: "Downloaded",
        size: "Size",
        eta: "ETA",
      },
    },
  },
  zh: {
    translation: {
      appName: "极速下载器",
      nav: {
        all: "全部任务",
        downloading: "进行中",
        completed: "已完成",
        paused: "已暂停",
        error: "失败",
      },
      header: {
        newDownload: "新建下载",
        search: "搜索下载任务…",
        playlist: "播放列表下载",
      },
      update: {
        title: "检查更新",
        checking: "正在检查更新…",
        upToDate: "已是最新版本",
        downloadNow: "发现新版本",
        currentVersion: "当前版本",
        latestVersion: "最新版本",
        releaseNotes: "更新内容",
        downloadUpdate: "下载更新",
        downloadStarted: "更新包已开始下载",
        openPage: "打开发布页",
        retry: "重试",
        failed: "检查更新失败",
      },
      action: {
        pause: "暂停",
        resume: "继续",
        cancel: "取消",
        remove: "删除",
        openFolder: "打开文件夹",
        retry: "重试",
        start: "开始下载",
        download: "下载",
        browse: "浏览",
        copy: "复制",
        close: "关闭",
        save: "保存",
        verify: "校验文件",
      },
      status: {
        Pending: "待确认",
        Queued: "排队中",
        Downloading: "下载中",
        Paused: "已暂停",
        Completed: "已完成",
        Error: "出错",
        Canceled: "已取消",
      },
      task: {
        segments: "{{n}} 个连接",
        unknownSize: "未知大小",
        error: "错误",
        hash: "哈希值",
        hashMatched: "校验通过",
        hashMismatch: "校验失败",
      },
      error: {
        forbidden: "服务器拒绝访问 (403)，可能被 Cloudflare 反爬拦截",
        notFound: "文件不存在 (404)",
        invalidUrl: "请输入有效的 http(s) 链接",
        stalled: "连接停滞无数据（超时）",
        cancelled: "下载已取消",
        taskMissing: "任务不存在",
      },
      verify: {
        algorithm: "算法",
        expectedHash: "期望哈希值",
        optional: "留空则自动从伴生文件读取",
        computing: "正在计算哈希值…",
      },
      dialog: {
        title: "新建下载",
        grabTitle: "确认下载",
        url: "下载地址",
        urlPlaceholder: "https://example.com/file.zip",
        filename: "文件名",
        filenameHint: "自动从服务器识别",
        saveTo: "保存位置",
        segments: "连接数",
        referer: "来源页面 (可选)",
        customHeaders: "自定义请求头 (可选)",
        videoQuality: "画质选择",
        writeSubs: "字幕",
        enableSubs: "启用字幕下载",
        detectSubs: "自动检测",
        invalidUrl: "请输入有效的 http(s) 链接",
        added: "已添加下载任务",
      },

      playlist: {
        title: "播放列表下载",
        fetch: "获取",
        found: "找到 {{n}} 个视频",
        noVideos: "播放列表中未找到视频",
        selectAll: "全选",
        deselectAll: "取消全选",
        selected: "已选择",
        downloadSelected: "下载选中",
        added: "{{n}} 个视频已添加到下载队列",
        hint: "在上方粘贴 YouTube 播放列表或频道链接",
      },
      settings: {
        title: "设置",
        saveDir: "下载目录",
        maxConcurrent: "最大并发下载数",
        defaultSegments: "每个文件的默认连接数",
        speedLimit: "速度限制",
        speedLimitKbps: "KB/s（0 为不限速）",
        proxy: "代理",
        proxySystem: "系统代理",
        proxyNone: "直连",
        proxyCustom: "自定义",
        proxyHint: "支持 http(s):// 与 socks5://（如 http://127.0.0.1:7890）",
        language: "语言",
        theme: "主题",
        accent: "强调色",
        appearance: "外观",
        downloads: "下载",
        behavior: "行为",
        duplicatePolicy: "文件已存在时",
        dupRename: "自动重命名（加序号）",
        dupOverwrite: "覆盖",
        dupSkip: "跳过下载",
        sortByType: "按类型自动归档到分类子目录（视频/音频/文档/…）",
        notifyComplete: "下载完成时显示通知",
        openFolderOnComplete: "下载完成后自动打开文件夹",
        themeDark: "深色",
        themeLight: "浅色",
        extension: "Chrome 扩展",
        extensionDesc: "安装扩展后，可直接在浏览器中一键抓取下载。",
        extensionSteps:
          "1. 下载扩展 zip · 2. 解压 · 3. 打开 chrome://extensions · 4. 开启开发者模式 · 5. 加载已解压的扩展程序 → 选择解压后的文件夹",
        extensionInstall: "一键安装扩展",
        extensionDownload: "从 GitHub 下载",
        extensionInstalled:
          "扩展已就绪，路径已复制到剪贴板。在浏览器扩展页点击「加载已解压的扩展程序」并粘贴即可。",
        extensionFail: "扩展安装失败",
        extensionOpenFail: "无法打开扩展页（未检测到 Chrome/Edge）",
        extensionNoBrowser: "未检测到",
        extensionBrowsers: "已检测浏览器：",
        openExtensionsPage: "打开浏览器扩展页",
        extensionPath: "扩展目录",
        apiToken: "API 令牌",
        apiTokenHint: "（粘贴到浏览器扩展中）",
        copyToken: "复制",
        apiTokenCopied: "API 令牌已复制",
        saved: "设置已保存",
      },
      theme: {
        dark: "深色",
        light: "浅色",
        system: "跟随系统",
      },
      videoQuality: {
        best: "最佳（视频+音频）",
        "2160p": "4K (2160p)",
        "1080p": "1080p",
        "720p": "720p",
        "480p": "480p",
        "360p": "360p",
        video: "仅视频",
        audio: "仅音频",
      },
      empty: {
        title: "暂无下载任务",
        hint: "点击「新建下载」或使用 Chrome 扩展从网页抓取文件。",
      },
      drop: {
        hint: "松开以添加下载链接",
      },
      toast: {
        paused: "下载已暂停",
        resumed: "下载已继续",
        canceled: "下载已取消",
        removed: "任务已删除",
        error: "操作失败",
        saved: "设置已保存",
      },
      title: {
        speed: "速度",
        downloaded: "已下载",
        size: "大小",
        eta: "剩余时间",
      },
    },
  },
};

i18n.use(initReactI18next).init({
  resources,
  lng: "zh",
  fallbackLng: "en",
  interpolation: { escapeValue: false },
});

export function setAppLanguage(lang: string) {
  if (lang === "en" || lang === "zh") {
    i18n.changeLanguage(lang);
  }
}

export default i18n;